...
> **Note**: You can skip all quantification methods using `--skip_quantification_method` if you only need alignment results. See the [Normalization and QC options](#normalization-and-qc-options) section for more details.
...
## Normalization and QC options

The pipeline provides several options for controlling quantification and downstream normalization/QC analysis.

### Skipping quantification

You can skip quantification methods for all alignment approaches using the `--skip_quantification_method` parameter. When enabled, alignment will proceed but quantification and downstream analysis will be skipped:

```bash
nextflow run pdichiaro/rnaseq --skip_quantification_method
```

This parameter affects:
- STAR-based quantification (RSEM, Salmon)
- HISAT2-based quantification (featureCounts)
- Pseudo-alignment quantification (Salmon, Kallisto)
- Additional genome counting modules

### Normalization methods

The pipeline supports two normalization methods for DESeq2 QC analysis:

#### All genes normalization (default)

Uses DESeq2's standard median-of-ratios normalization across all genes:

```bash
nextflow run pdichiaro/rnaseq --normalization_method all_genes
```

#### Invariant genes normalization

Uses GeneralNormalizer with invariant gene identification for more robust normalization:

```bash
nextflow run pdichiaro/rnaseq --normalization_method invariant_genes
```

When using invariant genes normalization, you can control the selection criteria:

- `--sigma_times`: Number of standard deviations for invariant gene selection (default: 1)
- `--n_pop`: Number of populations for population-based normalization (default: 1)
...
The pipeline provides several options to skip specific analysis steps:

| Parameter | Description |
|-----------|-------------|
| `--skip_alignment` | Skip alignment steps (use with pre-aligned BAM files) |
| `--skip_pseudo_alignment` | Skip pseudo-alignment workflows (Salmon/Kallisto) |
| `--skip_quantification_method` | Skip all quantification methods while keeping alignment |
| `--skip_qc` | Skip all QC steps except MultiQC |
| `--skip_fastqc` | Skip FastQC quality control |
| `--skip_multiqc` | Skip MultiQC report generation |
| `--skip_trimming` | Skip adapter trimming |
| `--skip_markduplicates` | Skip duplicate marking with Picard |
| `--skip_stringtie` | Skip StringTie transcript assembly |
| `--skip_bigwig` | Skip BigWig file generation |
| `--skip_deseq2_qc` | Skip DESeq2 PCA and heatmap plotting |
| `--skip_biotype_qc` | Skip additional featureCounts biotype QC |
| `--skip_rseqc` | Skip RSeQC quality control |
| `--skip_qualimap` | Skip Qualimap quality control |
| `--skip_dupradar` | Skip dupRadar analysis |
| `--skip_preseq` | Skip Preseq complexity analysis |

Example of skipping multiple steps:
