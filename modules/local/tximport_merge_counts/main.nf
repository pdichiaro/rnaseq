process TXIMPORT_MERGE_COUNTS {
    ...
    tuple val(meta3), path(gene_counts_length_scaled)
    tuple val(meta4), path(gene_counts_scaled)
    tuple val(meta5), path(gene_lengths)
    tuple val(meta6), path(transcript_counts)
    tuple val(meta7), path(transcript_tpm)
    tuple val(meta8), path(transcript_lengths)
    ...
    path "tximport.merged.gene_counts_length_scaled.tsv", emit: counts_gene_length_scaled
    path "tximport.merged.gene_counts_scaled.tsv"       , emit: counts_gene_scaled
    ...
    tximport_merge_counts.r \\
        ...
        --gene_counts_scaled ${gene_counts_scaled} \\
    ...
    touch ${prefix}.merged.gene_counts_length_scaled.tsv
    touch ${prefix}.merged.gene_counts_scaled.tsv
