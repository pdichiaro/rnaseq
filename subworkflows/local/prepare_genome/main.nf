...
include { ANNOTATION_MATRIX                    } from '../../../modules/local/annotation_matrix'
...
workflow PREPARE_GENOME {
    ...
    // This section will be moved after chrom_sizes is defined
    ...
    //----------------------------------------
    // 7.5) Prepare GTF annotations (PREP_GTF)
    //----------------------------------------
    ch_txdb_sqlite = Channel.empty()
    ch_metadata = Channel.empty()
    ch_annotation_matrix = Channel.empty()
    ch_genomic_features = Channel.empty()
    ch_annotation_summary = Channel.empty()
    
    if (ch_gtf && ch_chrom_sizes) {
        PREP_GTF(ch_gtf, ch_chrom_sizes)
        ch_txdb_sqlite = PREP_GTF.out.txdb_sqlite
        ch_metadata    = PREP_GTF.out.metadata
        ch_versions    = ch_versions.mix(PREP_GTF.out.versions)
        
        // Create annotation matrix from TxDB
        ANNOTATION_MATRIX(ch_txdb_sqlite, ch_metadata)
        ch_annotation_matrix = ANNOTATION_MATRIX.out.annotation_matrix
        ch_genomic_features  = ANNOTATION_MATRIX.out.genomic_features
        ch_annotation_summary = ANNOTATION_MATRIX.out.summary_stats
        ch_versions = ch_versions.mix(ANNOTATION_MATRIX.out.versions)
    }
    ...
    if (prepare_tool_indices.intersect(['star'])) {
        if (star_index) {
    ...
    txdb_sqlite      = ch_txdb_sqlite            // channel: path(TxDB.sqlite) from PREP_GTF
    metadata         = ch_metadata               // channel: path(metadata.txt) from PREP_GTF
    annotation_matrix = ch_annotation_matrix     // channel: path(annotation_matrix.txt) from ANNOTATION_MATRIX
    genomic_features = ch_genomic_features       // channel: path(genomic_features.rds) from ANNOTATION_MATRIX
    annotation_summary = ch_annotation_summary   // channel: path(summary_stats.txt) from ANNOTATION_MATRIX
