...
include { SAMTOOLS_INDEX as SAMTOOLS_INDEX_KALLISTO            } from '../../modules/nf-core/samtools/index'
...
workflow RNASEQ {
    ...
    def valid_normalization_methods = ['all_genes', 'invariant_genes']
    ...
    if (params.quantification && !valid_quantifications.contains(params.quantification)) {
        error "Invalid quantification '${params.quantification}'. Valid options: ${valid_quantifications.join(', ')}"
    }
    ...
    if (params.aligner == 'hisat2' && params.quantification && params.quantification != 'genome') {
        error "HISAT2 aligner only supports 'genome' quantification. HISAT2 does not generate transcriptome BAM files required for RSEM or Salmon quantification."
    }
    ...
    //
    if (params.aligner == 'star' && params.quantification && !params.skip_quantification_method) {
        
        if (params.quantification == 'rsem') {
            //
            ...
            if (!params.skip_qc & !params.skip_deseq2_qc) {
                if (params.normalization_method == 'invariant_genes') {
                    ...
                    ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES_ALIGNMENT.out.pca_multiqc.collect())
                    ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES_ALIGNMENT.out.dists_multiqc.collect())
                    ch_versions = ch_versions.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES_ALIGNMENT.out.versions)
                    ...
                    ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_ALL_GENES_ALIGNMENT.out.pca_multiqc.collect())
                    ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_ALL_GENES_ALIGNMENT.out.dists_multiqc.collect())
                    ch_versions = ch_versions.mix(NORMALIZE_DESEQ2_QC_ALL_GENES_ALIGNMENT.out.versions)
                    ch_scaling_factors = ch_scaling_factors.mix(NORMALIZE_DESEQ2_QC_ALL_GENES_ALIGNMENT.out.scaling_factors)
                }
            }

        } else if (params.quantification == 'salmon') {
            //
            ...
            if (!params.skip_qc & !params.skip_deseq2_qc) {
                if (params.normalization_method == 'invariant_genes') {
                    ...
                    ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES.out.pca_multiqc.collect())
                    ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES.out.dists_multiqc.collect())
                    ch_versions = ch_versions.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES.out.versions)
                    ch_scaling_factors = ch_scaling_factors.mix(NORMALIZE_DESEQ2_QC_INVARIANT_GENES.out.scaling_factors)
                } else {
                    NORMALIZE_DESEQ2_QC_ALL_GENES_ALIGNMENT (
                        QUANTIFY_STAR_SALMON.out.counts_gene.map { it[1] },
                        ch_pca_header_multiqc,
        ...
        } else if (params.quantification == 'genome') {
            //
    ...
    //
    // MODULE: Genome counting
    //
    if (!params.skip_quantification_method && ch_genomic_features) {
        GENOME_COUNT (
            ch_genome_bam.join(ch_genome_bam_index, by: [0]),
            ch_genomic_features
        )
    ...
    if (!params.skip_deeptools_norm && !params.skip_qc && !params.skip_deseq2_qc) {
        // Only run if we have scaling factors from normalization and BAM files available
        ch_scaling_factors
            .ifEmpty { error "No scaling factors available for deeptools normalization. Make sure DESeq2 QC is enabled and produces scaling factors." }
            .first()
            .set { ch_scaling_factors_file }
        
        // Check if we have BAM files (from alignment or pseudoalignment)
        ch_genome_bam
            .ifEmpty { error "No BAM files available for deeptools normalization. Make sure alignment or pseudoalignment with BAM generation is enabled." }
            .set { ch_bam_for_deeptools }
        
        DEEPTOOLS_BIGWIG_NORM (
            ch_bam_for_deeptools.join(ch_genome_bam_index, by: [0]),
            ch_scaling_factors_file
        )
        ch_versions = ch_versions.mix(DEEPTOOLS_BIGWIG_NORM.out.versions)
    }
    ...
    if (!params.skip_pseudo_alignment && params.pseudo_aligner) {
        ...
        if (params.pseudo_aligner == 'kallisto') {
            ...
            // Index Kallisto pseudoBAM files for BigWig generation
            SAMTOOLS_INDEX_KALLISTO (
                QUANTIFY_PSEUDO_ALIGNMENT.out.bam
            )
            ch_kallisto_bam_index = params.bam_csi_index ? SAMTOOLS_INDEX_KALLISTO.out.csi : SAMTOOLS_INDEX_KALLISTO.out.bai
            ch_genome_bam_index = ch_genome_bam_index.mix(ch_kallisto_bam_index)
            ch_versions = ch_versions.mix(SAMTOOLS_INDEX_KALLISTO.out.versions.first())
        }
        ...
        if (!params.skip_qc & !params.skip_deseq2_qc) {
            if (params.normalization_method == 'invariant_genes') {
                ...
                ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_ALL_GENES.out.pca_multiqc.collect())
                ch_multiqc_files = ch_multiqc_files.mix(NORMALIZE_DESEQ2_QC_ALL_GENES.out.dists_multiqc.collect())
                ch_versions = ch_versions.mix(NORMALIZE_DESEQ2_QC_ALL_GENES.out.versions)
                ch_scaling_factors = ch_scaling_factors.mix(NORMALIZE_DESEQ2_QC_ALL_GENES.out.scaling_factors)
            }
        }
    }
