...
for (n_top_var in ntop) {
    ...
    pl <- ggplot(pca.data, aes(PC1, PC2, label=paste0(" ", sample, " "))) +
        ...
        labs(title = paste0("First PCs on ", vst_name, "-transformed data (All genes normalization)"), subtitle = plot_subtitle) +
...
pheatmap(
    ...
    col=colors,
    main=paste("Euclidean distance between", vst_name, "of samples (All genes normalization)")
)
