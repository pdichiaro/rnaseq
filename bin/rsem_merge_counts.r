...
# R function to merge RSEM counts from multiple samples with advanced annotation integration
option_list <- list(
...
cat("=== RSEM MERGE WITH ADVANCED ANNOTATION ===\n")
...
# Function to process and merge RSEM data with annotation (using tximport logic)
process_rsem_file <- function(data, file_type, annotation_data) {
    cat("Processing", file_type, "with advanced merge logic...\n")
...
cat("\n=== Processing gene-level data with advanced annotation merge ===\n")

# Apply advanced merge logic to gene counts
final_gene_counts <- process_rsem_file(gene_counts, "gene_counts", annotation_data)
final_gene_tpm <- process_rsem_file(gene_tpm, "gene_tpm", annotation_data)
...
cat("\n=== Processing transcript-level data (no annotation merge for transcripts) ===\n")

# Save transcript results (no annotation merge for transcripts as annotation is gene-level)
# But still add gene_ids column and gene_name fallback
transcript_counts_processed <- process_rsem_file(transcript_counts, "transcript_counts", NULL)
transcript_tpm_processed <- process_rsem_file(transcript_tpm, "transcript_tpm", NULL)

write.table(transcript_counts_processed, 
           file = paste0(opt$output, ".merged.transcript_counts.tsv"), 
           sep = "\t", 
           quote = FALSE, 
           row.names = FALSE)

write.table(transcript_tpm_processed, 
           file = paste0(opt$output, ".merged.transcript_tpm.tsv"), 
           sep = "\t", 
           quote = FALSE, 
           row.names = FALSE)

cat("\n=== RSEM MERGE COMPLETED SUCCESSFULLY ===\n")
